...
Now to train this GMM on some data $\vec X = \{\vec x_1,\vec  x_2 .. , \vec x_n\}$ in the conventional way, we can calculate $\boldsymbol{\theta}$ using the 
...
We implemented this using pytorch's \cite{pytorch} autograd capabilities to compute the gradients.
...
Note that for simplicity we only use one random vector $v_i$ (one slice) for each data point. 
...
We implemented both Algorithms again using PyTorch \cite{pytorch} and torch.autograd. 
...
principles as in our GMM apply but due to the deeper structure, the model is much more complex and can approximate more complex distribution. 
It is also much larger and training takes longer. 
